...
function getPizzaOrder() {
	...
	// Get topping options
	if (pizza.toppings != "") {
		// Separate each option, and replace last comma if inputed
		pizza.toppings = pizza.toppings.trim().replace(/,\s*$/, "").split(",");
		// Remove last comma if inputed
	...
	// Set if extra cheese is wanted
	if (pizza.extraCheese === true) {
		// Calculate cost of extra cheese
